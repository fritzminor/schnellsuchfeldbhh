      ...
      switch (status) {
        case 0:
          if (!row.getCell(1).text.startsWith("Übersicht"))
            throw new Error(
              "Datei nicht erkannt, fängt nicht mit Übersicht an."
            );
          status = 1;
          break;
        case 1:
          if (row.getCell(1).text === "Haushaltsstelle") {
            if (
              !(
                row.getCell(2).text === "FKZ" &&
                row.getCell(4).text === "Zweckbestimmung" &&
                row.getCell(5).text.startsWith("Soll ")
              )
            ) {
              console.log("Spaltenfehler", row);
              throw new Error("Spalten nicht erkannt.");
            }
            firstYear = parseFloat(
              row.getCell(5).text.substr("Soll ".length)
            );
            status = 2;
          }
          break;
        ...
        case 3: {
          ...
          if (!haushaltsstelleArray[5]) {
            // ignore "apl" or "apl AR"
            const hhst: HHSt = {
              type: "hhst",
              ...
              expense:
                haushaltsstelleArray[3].charAt(0) >= "4",
              sollJahr1:
                typeof sollJahr1 === "number"
                  ? sollJahr1
                  : 0
            };
            hhsts.push(hhst);
