...
export function getClickSelectData(
  appState: AppState
): ClickSelectData {
  ...
  categories.forEach((category) => {
    ...
    if (parentCategory) {
      ...
      else deleteCurrCat = true;
    }

    if (
      (data[category] as ClickSelectCategoryData)
        .itemsSorted.length <= 1
    ) {
    ...
    if (deleteCurrCat) {
      ...
      if (catIndex >= 0) rootCategories.splice(catIndex, 1);
    }
